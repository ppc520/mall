...
import com.hdkj.mall.system.vo.SystemGroupDataRechargeConfigVo;
...
//        WeChatAuthorizeLoginUserInfoResponse userInfo = weChatService.getUserInfo(response.getOpenId(), response.getAccessToken());
     ...
     */
    @Override
    public UserRechargeResponse getTranferConfig() {
        UserRechargeResponse userRechargeResponse = new UserRechargeResponse();
        String rechargeAttention = systemConfigService.getValueByKey(Constants.CONFIG_TRANSFER_ATTENTION);
        List<String> rechargeAttentionList = new ArrayList<>();
        if(StringUtils.isNotBlank(rechargeAttention)){
            rechargeAttentionList = MallUtil.stringToArrayStrRegex(rechargeAttention, "\n");
        }
