...
public class StoreOrderTaskServiceImpl implements StoreOrderTaskService {
     ...
     */
    @Override
    public Boolean refundOrder_fw(UserLjhsOrder userLjhsOrder) {


        if (userLjhsOrder.getPayType1()!=1) {
            throw new MallException("余额支付，无法维修无法退款");
        }
        if (new BigDecimal(userLjhsOrder.getPayPrice()).compareTo(BigDecimal.ZERO) <= 0) {
            throw new MallException("退款金额必须大于0");
        }
        ...
        try {
            ...
            request.setType(1);
            request.setAmount(new BigDecimal(userLjhsOrder.getPayPrice()));
            StoreOrder storeOrder = new StoreOrder();
            storeOrder.setOrderId(userLjhsOrder.getOrderId());
            storeOrder.setIsChannel(1);
            storeOrder.setPayPrice(new BigDecimal(userLjhsOrder.getPayPrice()));
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            userLjhsOrder.setRefundStatus(3);
            userLjhsOrderService.updateById(userLjhsOrder);
    ...
    public Boolean refundOrder(StoreOrder storeOrder) {
        ...
        integralRecordList.forEach(record -> {
            if (record.getType().equals(IntegralRecordConstants.INTEGRAL_RECORD_TYPE_SUB)) {// 订单抵扣部分
                user.setIntegral(user.getIntegral() + Integer.valueOf(record.getIntegral()));
                record.setId(null);
                ...
                record.setBalance(String.valueOf(user.getIntegral()));
