...
import com.hdkj.mall.seckill.service.StoreSeckillService;
import com.hdkj.mall.store.dao.StoreProductDao;
import com.hdkj.mall.store.model.*;
import com.hdkj.mall.store.request.StoreProductAttrValueRequest;
import com.hdkj.mall.store.request.StoreProductSearchRequest;
import com.hdkj.mall.store.request.StoreProductStockRequest;
import com.hdkj.mall.store.response.StoreProductAttrValueResponse;
...
import com.hdkj.mall.task.order.OrderRefundTask;
    ...
    */
    ...
    public PageInfo<StoreProductResponse> getList(StoreProductSearchRequest request, PageParamRequest pageParamRequest) {
        ...
        if(request.getProductType()!=null){
            lambdaQueryWrapper.eq(StoreProduct::getProductType, request.getProductType());
        }
    ...
    public boolean save(StoreProductRequest storeProductRequest) {
        ...
        if (storeProductRequest.getSpecType()) {
            if (CollUtil.isEmpty(storeProductRequest.getAttr())) {
                throw new MallException("商品属性不能为空");
            }
            if (CollUtil.isEmpty(storeProductRequest.getAttrValue())) {
                throw new MallException("商品属性详情不能为空");
            }
            // 校验商品属性详情参数
...
//        }
     ...
     */
    @Override
    public List<StoreProductTabsHeader> getTabsHeader(StoreProductSearchRequest storeProductSearchRequest) {
        ...
        for (StoreProductTabsHeader h : headers){
            LambdaQueryWrapper<StoreProduct> lambdaQueryWrapper = new LambdaQueryWrapper<>();
            lambdaQueryWrapper.eq(StoreProduct::getProductType,0);
            if(storeProductSearchRequest.getMerId()!=null && storeProductSearchRequest.getMerId() !=0){
                lambdaQueryWrapper.eq(StoreProduct::getMerId, storeProductSearchRequest.getMerId());
            }
            switch (h.getType()){
                case 1:
                    //出售中（已上架）
     ...
     */
    @Override
    public List<StoreProductTabsHeader> getTabsFwHeader(StoreProductSearchRequest storeProductSearchRequest,Integer type) {
        List<StoreProductTabsHeader> headers = new ArrayList<>();
        StoreProductTabsHeader header1 = new StoreProductTabsHeader(0,"出售中品类",1);
        StoreProductTabsHeader header2 = new StoreProductTabsHeader(0,"仓库中品类",2);
//        StoreProductTabsHeader header3 = new StoreProductTabsHeader(0,"已经售馨商品",3);
//        StoreProductTabsHeader header4 = new StoreProductTabsHeader(0,"警戒库存",4);
        StoreProductTabsHeader header5 = new StoreProductTabsHeader(0,"品类回收站",5);
        headers.add(header1);
        headers.add(header2);
//        headers.add(header3);
//        headers.add(header4);
        ...
        for (StoreProductTabsHeader h : headers){
            LambdaQueryWrapper<StoreProduct> lambdaQueryWrapper = new LambdaQueryWrapper<>();
            lambdaQueryWrapper.eq(StoreProduct::getProductType,type);
            if(storeProductSearchRequest.getMerId()!=null && storeProductSearchRequest.getMerId() !=0){
                lambdaQueryWrapper.eq(StoreProduct::getMerId, storeProductSearchRequest.getMerId());
            }
            switch (h.getType()){
    ...
    public List<StoreProduct> getIndexProduct(Integer type, PageParamRequest pageParamRequest) {
        PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<StoreProduct> lambdaQueryWrapper = Wrappers.lambdaQuery();
        lambdaQueryWrapper.select(StoreProduct::getId, StoreProduct::getImage, StoreProduct::getStoreName, StoreProduct::getProductType,
                StoreProduct::getPrice, StoreProduct::getOtPrice, StoreProduct::getActivity, StoreProduct::getDelivery);
        switch (type){
    ...
    public List<StoreProduct> findH5List(ProductRequest request, PageParamRequest pageRequest) {
        ...
        // id、名称、图片、价格、销量、活动
        lqw.select(StoreProduct::getId, StoreProduct::getStoreName, StoreProduct::getImage, StoreProduct::getPrice, StoreProduct::getProductType,
                StoreProduct::getActivity, StoreProduct::getSales, StoreProduct::getFicti, StoreProduct::getUnitName, StoreProduct::getDelivery);

        if(request.getProductType()!=null){
            lqw.eq(StoreProduct::getProductType, request.getProductType());
        }else {
            lqw.eq(StoreProduct::getProductType, 0);
        }
...
//        lqw.eq(StoreProduct::getMerId, false);
    ...
    public StoreProduct getH5Detail(Integer id) {
        LambdaQueryWrapper<StoreProduct> lqw = Wrappers.lambdaQuery();
        lqw.select(StoreProduct::getId, StoreProduct::getImage, StoreProduct::getStoreName, StoreProduct::getSliderImage, StoreProduct::getDelivery,
                StoreProduct::getOtPrice, StoreProduct::getStock, StoreProduct::getSales, StoreProduct::getPrice, StoreProduct::getActivity,
                StoreProduct::getFicti, StoreProduct::getIsSub, StoreProduct::getStoreInfo, StoreProduct::getBrowse, StoreProduct::getUnitName, StoreProduct::getProductType);
