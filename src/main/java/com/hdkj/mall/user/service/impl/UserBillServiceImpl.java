...
import com.hdkj.mall.user.dao.UserBillDao;
...
public class UserBillServiceImpl extends ServiceImpl<UserBillDao, UserBill> implements UserBillService {
    ...
    */
    ...
    public boolean saveRefundBill(StoreOrderRefundRequest request, User user) {
        ...
        userBill.setMark("订单退款" + request.getAmount() + "元");
        userBill.setPm(1);
     ...
     */
    @Override
    public boolean saveRefundBill_fw(UserLjhsOrder request, User user) {
        UserBill userBill = new UserBill();
        userBill.setTitle("服务订单退款");
        userBill.setUid(user.getUid());
        userBill.setCategory(Constants.USER_BILL_CATEGORY_MONEY);
        userBill.setType(Constants.USER_BILL_TYPE_PAY_SERVER_ORDER_REFUND);
        userBill.setNumber(new BigDecimal(request.getPayPrice()));
        userBill.setLinkId(request.getOrderId().toString());
        userBill.setMark("订单退款" + request.getPayPrice() + "元");
        userBill.setBalance(user.getNowMoney().add(new BigDecimal(request.getPayPrice())));
        userBill.setPm(1);
        return save(userBill);
    }
