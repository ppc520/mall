...
import com.hdkj.mall.front.request.UserBindingPhoneUpdateRequest;
import com.hdkj.mall.front.response.UserCenterResponse;
import com.hdkj.mall.front.response.UserSpreadPeopleItemResponse;
import com.hdkj.mall.front.service.LoginService;
import com.hdkj.mall.marketing.service.StoreCouponService;
import com.hdkj.mall.store.model.StoreOrder;
import com.hdkj.mall.store.request.RetailShopStairUserRequest;
...
import com.hdkj.mall.system.service.SystemUserLevelService;
import com.hdkj.mall.user.dao.ServiceUserDao;
...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public boolean updateIntegralMoney(UserOperateIntegralMoneyRequest request) {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (request.getIntegralValue() > 0) {
                ...
                integralRecord.setIntegral(String.valueOf(request.getIntegralValue()));
                integralRecord.setStatus(IntegralRecordConstants.INTEGRAL_RECORD_STATUS_COMPLETE);
                if (request.getIntegralType() == 1) {// 增加
                    integralRecord.setType(IntegralRecordConstants.INTEGRAL_RECORD_TYPE_ADD);
                    integralRecord.setBalance(String.valueOf(user.getIntegral() + request.getIntegralValue()));
    ...
    public UserCenterResponse getUserCenter() {
        ...
        lambdaQuery.eq(UserAttention::getUid,userCenterResponse.getUid());
        List<Integer> list1 = new ArrayList<>();
        list1.add(0);
        list1.add(1);
        lambdaQuery.in(UserAttention::getType,list1);
        userCenterResponse.setGznum(userAttentionDao.selectCount(lambdaQuery));

        LambdaQueryWrapper<UserAttention> lambdaQuery1 = Wrappers.lambdaQuery();
        lambdaQuery1.eq(UserAttention::getDid,userCenterResponse.getUid());
        userCenterResponse.setFsnum(userAttentionDao.selectCount(lambdaQuery1));
        return userCenterResponse;
    }
     ...
     */
    @Override
    public UserCenterResponse getUserCenterByid(Integer id) {
        ...
        LambdaQueryWrapper<UserAttention> lambdaQuery1 = Wrappers.lambdaQuery();
        lambdaQuery1.eq(UserAttention::getDid,userCenterResponse.getUid());
        List<UserAttention> list1 = userAttentionDao.selectList(lambdaQuery1);
        for (UserAttention userAttention : list1){
            if(getById(userAttention.getUid())==null){
                list1.remove(userAttention);
            }
        }
//        userCenterResponse.setFsnum(userAttentionDao.selectCount(lambdaQuery1));
        userCenterResponse.setFsnum(list1.size());

        User user1 = getInfo();
        UserAttention userAttention = new UserAttention();
        userAttention.setUid(user1.getUid());
        userAttention.setDid(currentUser.getUid());
        userAttention.setType(0);
        userCenterResponse.setIsgz(dynamicService.isUserAttention(userAttention));
        return userCenterResponse;
    }

    @Override
    public CategoryVo getAdminInfoById(Integer id) {
        ...
        LambdaQueryWrapper<UserAttention> lambdaQuery1 = Wrappers.lambdaQuery();
        lambdaQuery1.eq(UserAttention::getDid,category.getId());
        lambdaQuery1.eq(UserAttention::getType,1);
        List<UserAttention> list1 = userAttentionDao.selectList(lambdaQuery1);
        for (UserAttention userAttention : list1){
            if(getById(userAttention.getUid())==null){
                list1.remove(userAttention);
        ...
        categoryVo.setFsnum(list1.size());

        User user1 =getInfo();
        if(user1 ==null){
            categoryVo.setIsgz(false);
        }else{
            ...
            categoryVo.setIsgz(dynamicService.isUserAttention(userAttention));
        }
        return categoryVo;
    }
    ...
    public boolean spread(Integer currentUserId, Integer spreadUserId) {
        ...
        if (null == currentUser) throw new MallException("用户id:" + currentUserId + "不存在");
        User spreadUser = userDao.selectById(spreadUserId);
        if (null == spreadUser) throw new MallException("用户id:" + spreadUserId + "不存在");
        // 检查是否是推广员
        if (!spreadUser.getIsPromoter()) throw new MallException("用户id:" + spreadUserId + "不是推广员身份");
        // 检查是否已经有推广关系
    ...
    @Override
    public JSONObject checkSpecialCharactersFun(String content){
        ...
        try {
            ...
            StringBuffer sb = new StringBuffer("https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid="+programAppId+"&secret="+programAppSecret);
            JSONObject jsonObject = JSONUtil.parseObj(getURContent(sb.toString()));
            ...
            httpResponse =  HttpRequest.post("https://api.weixin.qq.com/wxa/msg_sec_check?access_token="+jsonObject.get("access_token").toString()) // url
                    .body(json.toString()) // json参数
            ...
            if(httpResponse.getStatus() == 200){
                ...
                throw new MallException("数据校验失败！");
            }

        } catch (Exception e) {
            e.printStackTrace();
            throw new MallException("参数错误，校验失败！");
        } finally{
            try {
                //释放连接
    ...
    public static String getURContent(String urlStr) {
        //请求的url
        URL url = null;
        //请求的输入流
     ...
     */
    @Override
    public List<ServiceUser> getListServiceUser(ServiceUser serviceUser) {
        LambdaQueryWrapper<ServiceUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(ServiceUser::getType, serviceUser.getType());
        lambdaQueryWrapper.eq(ServiceUser::getCid, serviceUser.getCid());
        return serviceUserDao.selectList(lambdaQueryWrapper);
    }
     ...
     */
    @Override
    public ServiceUser getInfoServiceUser(Integer id) {
        if(id ==null){
            throw new MallException("参数错误！");
        }
        return serviceUserDao.selectById(id);
    }
    /**
     * 修改
     */
    @Override
    public Boolean updateServiceUser(ServiceUser serviceUser) {
        if(serviceUser.getId() ==null){
            throw new MallException("参数错误！");
        }
        return serviceUserDao.updateById(serviceUser) > 0;
    }
     ...
     */
    @Override
    public Boolean saveServiceUser(ServiceUser serviceUser) {

        LambdaQueryWrapper<ServiceUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(ServiceUser::getType, serviceUser.getType());
        lambdaQueryWrapper.eq(ServiceUser::getCid, serviceUser.getCid());
        lambdaQueryWrapper.eq(ServiceUser::getUid, serviceUser.getUid());
        ServiceUser serviceUser1 = serviceUserDao.selectOne(lambdaQueryWrapper);
        if(serviceUser1!=null){
            return true;
        }else{
            return serviceUserDao.insert(serviceUser) > 0;
        }
    }
    /**
     * 删除
    ...
    public Boolean deleteServiceUser(Integer id) {
        ...
        return serviceUserDao.deleteById(id) > 0;
    }
    /**
     * 列表
     */
    @Override
    public PageInfo<ServiceUserResponse> getListServiceUser(ServiceUser request, PageParamRequest pageParamRequest) {
        Page<ServiceUserResponse> page = PageHelper.startPage(pageParamRequest.getPage(), pageParamRequest.getLimit());
        LambdaQueryWrapper<ServiceUser> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        lambdaQueryWrapper.eq(ServiceUser::getType, request.getType());
        lambdaQueryWrapper.eq(ServiceUser::getCid, request.getCid());
        List<ServiceUser> list =  serviceUserDao.selectList(lambdaQueryWrapper);
        List<ServiceUserResponse> serviceUserResponseArrayList = new ArrayList<>();
        for (ServiceUser serviceUser : list) {
            ServiceUserResponse serviceUserResponse = new ServiceUserResponse();
            BeanUtils.copyProperties(serviceUser, serviceUserResponse);
            User user  =getById(serviceUser.getUid());
            if(user!=null){
                serviceUserResponse.setUname(user.getNickname());
            }
            serviceUserResponseArrayList.add(serviceUserResponse);
        }
        return CommonPage.copyPageInfo(page, serviceUserResponseArrayList);
    }
