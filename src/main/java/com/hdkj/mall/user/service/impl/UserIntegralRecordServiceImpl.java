...
import com.hdkj.mall.user.dao.UserIntegralRecordDao;
...
public class UserIntegralRecordServiceImpl extends ServiceImpl<UserIntegralRecordDao, UserIntegralRecord> implements UserIntegralRecordService {
    ...
    public void integralThaw() {
        ...
        for (UserIntegralRecord record : thawList) {
            ...
            // 计算积分余额
            Integer balance = user.getIntegral() + Integer.valueOf(record.getIntegral());
            record.setBalance(String.valueOf(balance));

            // 解冻
            Boolean execute = transactionTemplate.execute(e -> {
                updateById(record);
                userService.operationIntegral(record.getUid(), Integer.valueOf(record.getIntegral()), user.getIntegral(), "add");
                return Boolean.TRUE;
            });
            if (!execute) {
    ...
    public Integer getSumIntegral(Integer uid, Integer type, String date, String linkType) {
        ...
        return Integer.valueOf(integralRecord.getIntegral());
    }
