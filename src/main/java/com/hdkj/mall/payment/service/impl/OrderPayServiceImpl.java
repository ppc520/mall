...
import com.hdkj.mall.front.vo.WxPayJsResultVo;
...
import com.hdkj.mall.store.model.StoreProductAttrValue;
...
//                storeBargainUserService.updateById(storeBargainUser);
     ...
     */
    @Override
    public OrderPayResultResponse payment1(OrderPayRequest orderPayRequest, String ip) {
        UserLjhsOrder userLjhsOrder = userLjhsOrderService.getByOderId(orderPayRequest.getOrderNo());
        ...
        OrderPayResultResponse response = new OrderPayResultResponse();
        response.setOrderNo(userLjhsOrder.getOrderId());
        response.setPayType("weixin");
        // 微信支付，调用微信预下单，返回拉起微信支付需要的信息
        if (response.getPayType().equals(PayConstants.PAY_TYPE_WE_CHAT)) {
            Map<String, String> unifiedorder = weChatPayService.unifiedorder1(userLjhsOrder, ip);
            response.setStatus(true);
            WxPayJsResultVo vo = new WxPayJsResultVo();
            vo.setAppId(unifiedorder.get("appId"));
            vo.setNonceStr(unifiedorder.get("nonceStr"));
    ...
    private UserIntegralRecord integralRecordSubInit(StoreOrder storeOrder, User user) {
        ...
        integralRecord.setIntegral(String.valueOf(storeOrder.getUseIntegral()));
        integralRecord.setBalance(String.valueOf(user.getIntegral()));
    ...
    private UserIntegralRecord integralRecordInit(StoreOrder storeOrder, Integer balance, Integer integral, String type) {
        ...
        integralRecord.setBalance(String.valueOf(balance));
